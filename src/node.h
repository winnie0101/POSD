...
public:
    ...
    virtual void rename(string name) {
        size_t slash = _path.rfind("/");
        string newPath = _path.substr(0, slash+1) + name;
        updatePath(newPath);

        string parentPathOfChildren = newPath;
        updateChildrenPath(parentPathOfChildren);
    }
    ...
    virtual void updateChildrenPath(string parentPath) {};
};
